...
// API keys — move to .env as REACT_APP_TENOR_KEY / REACT_APP_GIPHY_KEY
...
const sessionStore = {
  list() {
    ...
    } catch (e) { console.warn("sessionStore.list error", e); return []; }
  },
  get(id) {
    try { return JSON.parse(localStorage.getItem("rk_session_" + id)); } catch (e) { console.warn("sessionStore.get error", e); return null; }
  },
  save(session) {
    try {
    ...
    } catch (e) { console.warn("sessionStore.save error", e); }
  },
  delete(id) {
    try {
...
function getOrCreateDefaultSession() {
  ...
  const id = uid();
  const session = { id, name: "My Team Retro", sprintNumber: 13, date: "", cutoffDate: "", cutoffTime: "", q3Variant: 0 };
...
function relaxedLabel(ms) {
  if (ms === null) return { text: "No cutoff set", state: "open" };
  ...
  if (totalMin <= 45) return { text: `Closes in about ${mins} min`, state: "soon" };
...
function CountdownBar({ session }) {
  ...
  if (ms === null) return null; // no cutoff set — hide the bar entirely
  return (
...
const css = `
  ...
  :root {
    --or-red: #E8003D; --or-red-dark: #c0002f; --or-red-glow: rgba(232,0,61,.18);
    --bg: #0f1117; --bg-card: #181c27; --bg-raised: #1e2235; --bg-input: #242838;
    --border: #2a2f42; --border-light: #333849;
    --text: #e8eaf0; --text-muted: #8b92a8; --text-dim: #555e78;
    --white: #ffffff; --green: #10b981; --blue: #4d96ff;
  }
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .session-select { background: var(--bg-raised); border: 1.5px solid var(--border-light); border-radius: 8px; color: var(--text); font-family: 'DM Sans', sans-serif; font-size: 13px; font-weight: 600; padding: 6px 32px 6px 12px; cursor: pointer; outline: none; appearance: none; -webkit-appearance: none; max-width: 220px; }
  ...
  .submit-btn { width: 100%; padding: 15px; background: var(--or-red); color: white; border: none; border-radius: 6px; font-family: 'Libre Franklin', sans-serif; font-size: 17px; font-weight: 800; cursor: pointer; transition: all .2s; }
  ...
  .session-link-box h3 { font-family: 'Libre Franklin', sans-serif; font-size: 17px; font-weight: 800; color: var(--text); margin-bottom: 6px; }
  ...
  .closed-banner { background: rgba(100,100,100,.12); border: 1px solid #444; border-radius: 10px; padding: 14px 18px; margin-bottom: 20px; font-size: 14px; color: var(--text-muted); text-align: center; }
  ...
  .q1-mode-toggle { display: flex; align-items: center; margin-bottom: 18px; background: var(--bg-raised); border-radius: 8px; padding: 3px; width: fit-content; }
...
async function searchGifs(query) {
  ...
  } catch (e) { if (e.name !== "AbortError") console.warn("Tenor error", e); clearTimeout(timeout); }
  try {
...
function GifPicker({ value, onChange }) {
      ...
      <div>
        <div className="gif-selected-preview">
          ...
          <div className="gif-selected-preview-text">
            <strong>GIF selected ✓</strong><span>{value.title || "Untitled"}</span>
  ...
  return (
    <div className="gif-picker">
      ...
      {!loading && searched && results.length === 0 && <div className="gif-empty">No GIFs found. Try different words!</div>}
      {!loading && results.length > 0 && (
        <><div className="gif-grid">{results.map(gif => (<div key={gif.id} className={`gif-item ${value?.id === gif.id ? "selected" : ""}`} onClick={() => selectGif(gif)}><img src={gif.images.fixed_height_small.url} alt={gif.title} loading="lazy" />{value?.id === gif.id && <div className="gif-check">✓</div>}</div>))}</div><div className="giphy-attr">Powered by GIPHY</div></>
      )}
  ...
  return (
    <div>
      <div className="q1-mode-toggle">
        ...
        <button className={`q1-mode-btn gif-mode ${mode === "gif" ? "active" : ""}`} onClick={() => handleModeSwitch("gif")}><span className={`toggle-switch ${mode === "gif" ? "on" : ""}`} />🎬 GIF</button>
      </div>
      {mode === "emoji" ? (
        <div className="emoji-grid">{EMOJIS.map(e => <button key={e} className={`emoji-btn ${value === e ? "selected" : ""}`} onClick={() => onChange(e)}>{e}</button>)}</div>
...
function StickyCard({ card, hidden, onGroup, grouped, groupName, revealed, currentUser, onVote }) {
  ...
  const handleVote = (e, dir) => { e.stopPropagation(); if (!revealed) return; onVote(card.id, myVote === dir ? 0 : dir); };
    ...
    <div className="sticky" style={{ background: card.color }} onClick={onGroup}>
      ...
      {card.type !== "emoji" && (
        <div className="sticky-content">
          {hidden ? "••••••••" : (typeof card.content === "string" && card.content.startsWith("<") ? <span dangerouslySetInnerHTML={{ __html: card.content }} /> : card.content)}
        </div>
      )}
      ...
      {revealed && (
        <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 8, paddingTop: 6, borderTop: "1px solid rgba(0,0,0,.1)" }} onClick={e => e.stopPropagation()}>
          <button onClick={e => handleVote(e, 1)} style={{ display: "flex", alignItems: "center", gap: 3, background: myVote === 1 ? "#ff4500" : "rgba(0,0,0,.1)", border: "none", borderRadius: 4, padding: "2px 7px", cursor: "pointer", fontSize: 12, fontWeight: 700, color: myVote === 1 ? "white" : "rgba(0,0,0,.55)", transition: "all .15s" }}>▲ {upCount > 0 ? upCount : ""}</button>
          <button onClick={e => handleVote(e, -1)} style={{ display: "flex", alignItems: "center", gap: 3, background: myVote === -1 ? "#7193ff" : "rgba(0,0,0,.1)", border: "none", borderRadius: 4, padding: "2px 7px", cursor: "pointer", fontSize: 12, fontWeight: 700, color: myVote === -1 ? "white" : "rgba(0,0,0,.55)", transition: "all .15s" }}>▼ {downCount > 0 ? downCount : ""}</button>
          {netScore !== 0 && <span style={{ fontSize: 11, fontWeight: 700, color: netScore > 0 ? "#ff4500" : "#7193ff", marginLeft: "auto" }}>{netScore > 0 ? "+" : ""}{netScore}</span>}
        </div>
      )}
    </div>
  );
  ...
  return (
    <div style={{ marginBottom: 12 }}>
      ...
      {loading ? <div className="ai-ideas-wrap"><div className="ai-ideas-loading"><div className="ai-dot-spin" />Thinking…</div></div>
        : ideas.length > 0 && <div className="ai-ideas-wrap"><div className="ai-ideas-header">✨ Starter ideas — click to use</div>{ideas.map((idea, i) => <button key={i} className="ai-idea-chip" onClick={() => onSelect(idea)}><span className="ai-idea-chip-text">{idea}</span><span className="ai-idea-use">Use this →</span></button>)}</div>}
    </div>
  );
}
...
function ConfirmModal({ answers, questions, onConfirm, onCancel }) {
  const getAnswerPreview = (q) => { const val = answers[q.id]; if (!val || val === "") return null; if (typeof val === "object" && val.url) return "GIF: " + (val.title || "selected"); return val; };
  return (
    <div className="modal-overlay">
      <div className="modal">
        ...
        <div className="confirm-summary">
          {questions.map(q => { const preview = getAnswerPreview(q); return (<div key={q.id} className="confirm-row"><span className="confirm-q-label" style={{ background: q.color }}>{q.label}</span><span className={`confirm-answer ${!preview ? "empty" : ""}`}>{preview || "No response"}</span></div>); })}
        </div>
        <div className="modal-btns"><button className="tool-btn" onClick={onCancel}>← Go Back</button><button className="tool-btn primary" onClick={onConfirm}>Yes, Submit →</button></div>
      </div>
    </div>
  );
}
...
function EmojiPopup({ onSelect, onClose }) {
  ...
  return (
    <div className="emoji-popup">
      ...
      <div className="emoji-popup-grid">{filtered.map((e, i) => <div key={i} className="emoji-popup-item" onClick={() => { onSelect(e); onClose(); }}>{e}</div>)}</div>
    </div>
  );
}
...
function RichTextEditor({ value, onChange, placeholder, injectText }) {
  ...
  useEffect(() => {
    ...
    editorRef.current.innerHTML = clean; onChange(clean); editorRef.current.focus();
    const range = document.createRange(); const sel = window.getSelection();
    range.selectNodeContents(editorRef.current); range.collapse(false); sel.removeAllRanges(); sel.addRange(range);
  }, [injectText]);
...
function SubmitView({ session, questions, currentUser, joinQ1 }) {
  ...
  return (
    <div className="submit-wrap">
      ...
      {!isOpen && <div className="closed-banner">🔒 Submissions are closed for this session. Contact your facilitator if you still need to submit.</div>}
      ...
      {isEditing && <div className="editing-banner">✏️ You're <strong style={{ color: "#4D96FF", marginLeft: 4 }}>editing</strong> your previous responses.</div>}
      {questions.map(q => (
        <div key={q.id} className="q-card" style={{ borderTopColor: q.color }}>
          <div className="q-label" style={{ color: q.color }}>{q.label}</div>
          <div className="q-prompt">{q.prompt}</div>
          {q.type === "emoji" ? <Q1Picker value={answers[q.id]} onChange={v => setAnswer(q.id, v)} /> : (
            <><AIIdeas question={q.prompt} onSelect={idea => injectIdea(q.id, idea)} /><RichTextEditor value={answers[q.id]} onChange={v => setAnswer(q.id, v)} placeholder="Type your response, or pick a starter above…" injectText={injected[q.id]} /></>
          )}
        </div>
      ))}
      <div className="submit-actions">
        {isEditing && <button className="exit-btn" onClick={handleExit}>← Exit without saving</button>}
        <button className="submit-btn" style={{ flex: 1 }} disabled={!name.trim() || !isOpen} onClick={() => setShowConfirm(true)}>
...
function BoardView({ session, members, questions, currentUser }) {
  ...
  const [cards, setCards] = useState(() => {
    try {
      ...
      if (subs.length > 0) {
        return subs.flatMap(sub => Object.entries(sub.answers).filter(([, val]) => val).map(([qId, content]) => ({ id: uid(), qId, content, author: sub.name, color: CARD_COLORS[Math.floor(Math.random() * CARD_COLORS.length)], groupId: null, type: qId === "q1" ? "emoji" : "text" })));
      }
  ...
  const handleVote = (cardId, dir) => {
    setVotes(prev => {
      ...
      if (dir === 0) { delete next[cardId][currentUser]; } else { next[cardId][currentUser] = dir; }
      saveVotes(next); return next;
    });
  };
  ...
  const addFreeCard = () => {
    ...
    const updated = [...freeCards, newCard]; setFreeCards(updated); saveFreeCards(updated);
  };
  ...
  return (
    <div className="board-wrap">
      <div className="presence-strip">
        ...
        {presence.map((p, i) => (<div key={i} style={{ display: "flex", alignItems: "center", gap: 5 }}><div className="presence-avatar" style={{ background: p.color }}>{p.name[0]}</div><span className="presence-name">{p.name}</span></div>))}
      </div>
      {revealed && <div className="reaction-bar"><span className="reaction-bar-label">React</span>{REACTION_EMOJIS.map(e => <button key={e} className="reaction-emoji-btn" onClick={() => dropReaction(e)}>{e}</button>)}</div>}
      ...
      <div style={{ padding: "16px 24px 24px" }}>
        <div className="board-toolbar">
          ...
          {!revealed ? <button className="tool-btn" onClick={() => setRevealed(true)}>👁 Reveal All Cards</button> : <button className="tool-btn green" onClick={() => setRevealed(false)}>🙈 Hide Cards</button>}
        ...
        {!revealed && <div style={{ background: "rgba(255,200,0,.07)", borderRadius: 10, padding: "10px 16px", marginBottom: 16, fontSize: 14, color: "#d4a700", border: "1px solid rgba(255,200,0,.15)" }}>🔒 Cards are hidden. Click <strong>Reveal All Cards</strong> to show responses to the team.</div>}
        {freeCards.length > 0 && <div style={{ fontSize: 12, color: "var(--text-dim)", marginBottom: 12 }}>💡 <strong style={{ color: "var(--text-muted)" }}>{freeCards.length} free card{freeCards.length !== 1 ? "s" : ""}</strong> on canvas — drag to reposition, double-click to edit</div>}
          ...
          <div style={{ display: "flex", gap: 20, alignItems: "flex-start", minWidth: "max-content", position: "relative" }}>
                    ...
                    {qGroups.map(([gid, gdata]) => {
                      ...
                      return (
                        <div key={gid} className="group-block">
                          <div className="group-label"><input value={gdata.name} onChange={e => setGroups(g => ({ ...g, [gid]: { ...g[gid], name: e.target.value } }))} /><span style={{ fontSize: 11, color: "#999" }}>{gCards.length} cards</span></div>
                          {gCards.map(c => <StickyCard key={c.id} card={c} hidden={!revealed} grouped groupName={gdata.name} onGroup={() => setGroupingCard(c)} revealed={revealed} currentUser={currentUser} onVote={handleVote} />)}
                        </div>
                      );
                    })}
                    {ungrouped.map(c => <StickyCard key={c.id} card={c} hidden={!revealed} onGroup={() => revealed && setGroupingCard(c)} revealed={revealed} currentUser={currentUser} onVote={handleVote} />)}
            ...
            <div style={{ width: 280, flexShrink: 0 }}>
              <div style={{ background: "#1e2235", border: "1px solid #333849", borderRadius: "12px 12px 0 0", padding: "14px 16px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                <div style={{ fontFamily: "'Libre Franklin', sans-serif", fontSize: 13, fontWeight: 700, color: "#e8eaf0" }}>✅ Action Items</div>
              </div>
              <div style={{ background: "rgba(255,255,255,.02)", border: "1px solid #333849", borderTop: "none", borderRadius: "0 0 12px 12px", padding: 12, minHeight: 200, display: "flex", flexDirection: "column", gap: 8 }}>
                ...
                <div style={{ marginTop: 4, display: "flex", flexDirection: "column", gap: 6 }}>
                  <input placeholder="New action item…" value={newAction} onChange={e => setNewAction(e.target.value)} onKeyDown={e => e.key === "Enter" && addAction()} style={{ width: "100%", border: "1.5px solid var(--border)", borderRadius: 8, padding: "7px 10px", fontFamily: "DM Sans, sans-serif", fontSize: 13, outline: "none", background: "var(--bg-input)", color: "var(--text)" }} />
                  <div style={{ display: "flex", gap: 6 }}>
                    <input placeholder="Owner" value={newOwner} onChange={e => setNewOwner(e.target.value)} onKeyDown={e => e.key === "Enter" && addAction()} style={{ flex: 1, border: "1.5px solid var(--border)", borderRadius: 8, padding: "7px 10px", fontFamily: "DM Sans, sans-serif", fontSize: 13, outline: "none", background: "var(--bg-input)", color: "var(--text)" }} />
        ...
        {groupingCard && (
          <div className="modal-overlay" onClick={() => setGroupingCard(null)}>
            <div className="modal" onClick={e => e.stopPropagation()}>
              ...
              <div className="sticky" style={{ background: groupingCard.color, marginBottom: 20 }}><div className="sticky-content">{groupingCard.content}</div></div>
              <p>Add to an existing group or create a new one:</p>
              {groupsForQ(groupingCard.qId).map(([gid, gdata]) => (<div key={gid} className="suggestion-item" style={{ border: "1px solid var(--border)", borderRadius: 8, marginBottom: 6 }} onClick={() => { applyGroup(groupingCard.id, gid); setGroupingCard(null); }}>📁 {gdata.name} <span className="suggestion-apply">Add here →</span></div>))}
              <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
                ...
                <button className="tool-btn primary" onClick={() => { if (!newGroupName.trim()) return; const gid = createGroup(groupingCard.qId, newGroupName); applyGroup(groupingCard.id, gid); setNewGroupName(""); setGroupingCard(null); }}>Create</button>
              </div>
              <div className="modal-btns">{groupingCard.groupId && <button className="tool-btn danger" onClick={() => { applyGroup(groupingCard.id, null); setGroupingCard(null); }}>Remove from group</button>}<button className="tool-btn" onClick={() => setGroupingCard(null)}>Cancel</button></div>
            </div>
          </div>
        )}
      </div>
      ...
      {sessions.map(s => {
        ...
        return (
          <div key={s.id} className="history-card" onClick={() => onLoadSession(s)}>
            ...
            <div className="history-meta">Sprint {s.sprintNumber}{s.date ? ` · ${new Date(s.date + "T12:00:00").toLocaleDateString("en-US", { month: "numeric", day: "numeric", year: "2-digit" })}` : ""}{cutoff ? ` · Cutoff ${cutoff.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}` : ""}</div>
...
function SettingsModal({ currentSession, onSave, onClose }) {
  ...
  const [form, setForm] = useState(() => { const s = currentSession || {}; return { name: s.name || "", sprintNumber: s.sprintNumber || 1, date: s.date || "", cutoffDate: s.cutoffDate || "", cutoffTime: s.cutoffTime || "", q3Variant: s.q3Variant ?? 0 }; });
  ...
  const selectSession = (s) => { setEditingId(s.id); setForm({ name: s.name, sprintNumber: s.sprintNumber, date: s.date || "", cutoffDate: s.cutoffDate || "", cutoffTime: s.cutoffTime || "", q3Variant: s.q3Variant ?? 0 }); setNewSessionLink(null); setSaved(false); };
  const handleNewSession = () => { setEditingId(null); setForm({ name: "", sprintNumber: 1, date: "", cutoffDate: "", cutoffTime: "", q3Variant: 0 }); setNewSessionLink(null); setSaved(false); };

  const handleSave = () => {
    const id = editingId || uid();
    const session = { id, name: form.name || "Untitled Session", sprintNumber: parseInt(form.sprintNumber) || 1, date: form.date, cutoffDate: form.cutoffDate, cutoffTime: form.cutoffTime, q3Variant: parseInt(form.q3Variant) || 0 };
    sessionStore.save(session);
    setSessions(sessionStore.list()); setEditingId(id);
    setNewSessionLink(sessionStore.getSessionUrl(id));
    setSaved(true); onSave(session);
    setTimeout(() => setSaved(false), 2000);
  };

  const handleDelete = (e, id) => { e.stopPropagation(); if (!window.confirm("Delete this session?")) return; sessionStore.delete(id); setSessions(sessionStore.list()); if (editingId === id) { setEditingId(null); setForm({ name: "", sprintNumber: 1, date: "", cutoffDate: "", cutoffTime: "", q3Variant: 0 }); } };
      ...
      <div className="modal" style={{ maxWidth: 560 }} onClick={e => e.stopPropagation()}>
        {locked ? (
          <div className="settings-lock">
            ...
            <div className="settings-pw-row"><input className="settings-pw-input" type="password" placeholder="Password" value={pw} onChange={e => { setPw(e.target.value); setPwError(false); }} onKeyDown={e => e.key === "Enter" && unlock()} autoFocus /><button className="tool-btn primary" onClick={unlock}>Unlock</button></div>
          ...
          <>
            <h2>⚙️ Sessions</h2><p>Create and manage retro sessions. Each gets its own shareable link.</p>
            ...
            <div className="settings-section">
              ...
              <div style={{ display: "flex", gap: 10 }}>
                ...
                <div className="settings-row" style={{ flex: 2 }}>
                  <label className="settings-label">Retro date</label>
                  <input className="settings-input" type="date" value={form.date} onChange={e => set("date", e.target.value)} />
                </div>
              </div>
              <div className="settings-section-title" style={{ marginTop: 8 }}>Submission Cutoff</div>
              <div className="settings-hint" style={{ marginBottom: 10 }}>Set to a few minutes before your retro starts. The countdown timer counts down to this time.</div>
              <div style={{ display: "flex", gap: 10 }}>
                <div className="settings-row" style={{ flex: 2 }}>
                  <label className="settings-label">Cutoff date</label>
                  <input className="settings-input" type="date" value={form.cutoffDate} onChange={e => set("cutoffDate", e.target.value)} />
                </div>
                <div className="settings-row" style={{ flex: 1 }}>
                  <label className="settings-label">Cutoff time</label>
                  <input className="settings-input" type="time" value={form.cutoffTime} onChange={e => set("cutoffTime", e.target.value)} />
                </div>
              </div>
              <div className="settings-row">
            ...
            {newSessionLink && (
              <div className="session-link-box">
                <h3>🔗 Session Link</h3>
                <p>Share this link with your team. Submissions close at the cutoff time you set.</p>
            ...
            <div className="modal-btns">
              ...
              {saved ? <span className="settings-saved">✓ Saved!</span> : <button className="tool-btn primary" onClick={handleSave}>{editingId ? "Save Changes" : "Create Session →"}</button>}
            </div>
          </>
        )}
      </div>
  ...
  return (
    <div className="join-wrap">
      <div className="join-card">
        ...
        {cutoff && <div style={{ fontSize: 12, color: "var(--text-muted)", marginBottom: 16, background: "var(--bg-raised)", borderRadius: 8, padding: "6px 12px", display: "inline-block" }}>⏰ Submissions close at {cutoff.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</div>}
        ...
        <div style={{ marginTop: 20 }}><button className="join-btn" disabled={!name.trim()} onClick={() => onJoin(name.trim(), q1Val)}>Join Session →</button></div>
        {joined.length > 0 && (
        ...
        <nav className="nav">
          <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 6, background: "var(--bg-raised)", borderRadius: 20, padding: "4px 12px 4px 4px", border: "1px solid var(--border)" }}>
              ...
              <button onClick={() => { try { localStorage.removeItem(`rk_name_${activeSession.id}`); } catch (e) { console.warn(e); } setCurrentUser(null); }} style={{ background: "none", border: "none", color: "var(--text-dim)", cursor: "pointer", fontSize: 13, padding: 0, marginLeft: 2 }} title="Leave session">✕</button>
            </div>
          </div>
